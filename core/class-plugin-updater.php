...
class Plugin_Updater {
	 ...
	 */
	public function __construct( $_api_url, $_plugin_file, $unique_key, $_api_data = null ) {
		...
		$this->failed_request_cache_key = 'edd_' . $this->unique_key . '_failed_http_' . md5( $this->api_url );
		 ...
		 */
		do_action( 'post_' . $this->unique_key . '_plugin_updater_setup', $edd_plugin_data );
	...
	public function get_repo_api_data() {
		...
		if ( false === $version_info ) {
			...
			if ( ! property_exists( $version_info, 'requires' ) || empty( $version_info->requires ) ) {
				$version_info->requires = '';
			}
			if ( ! property_exists( $version_info, 'requires_php' ) || empty( $version_info->requires_php ) ) {
	...
	public function show_update_notification( $file, $plugin ) {
		...
		if ( ! empty( $update_cache->response[ $this->name ]->sections->changelog ) ) {
			$changelog_link = add_query_arg(
				array(
					'stobokit_action' => 'view_plugin_changelog',
	 ...
	 *
	 * @see Plugin_Updater::request_recently_failed
	 *
	 * @since 1.9.1
	 */
	...
	private function get_version_from_remote() {
		 ...
		 */
		$api_params = apply_filters( 'edd_' . $this->unique_key . '_plugin_updater_api_params', $api_params, $this->api_data, $this->plugin_file );
	...
	public function set_version_info_cache( $value = '', $cache_key = '' ) {
		...
		delete_option( 'edd_' . $this->unique_key . '_api_request_' . md5( serialize( $this->slug . $this->api_data['license'] . $this->beta ) ) );
	}
	...
	private function verify_ssl() {
		return (bool) apply_filters( 'edd_' . $this->unique_key . '_api_request_verify_ssl', true, $this );
	}
	...
	private function get_cache_key() {
		...
		return 'edd_' . $this->unique_key . '_' . md5( serialize( $string ) );
	}
}
